    ...
    internal class GetCalendar
    {
        public static async Task<List<Event>> GetCalendarClass()
        {
            UserCredential credential = await GoogleWebAuthorizationBroker.AuthorizeAsync(
                new ClientSecrets
                {
                    ClientId = MainWindow.Config.ClientId,
                    ClientSecret = MainWindow.Config.ClientKey
                },
            ...
            var calendarsRaw = await service.Events
                .List("timderkuerbis@gmail.com")
                .ExecuteAsync()
                .ConfigureAwait(false);

            return calendarsRaw.Items
                .ToList()
                .Where(
                    x =>
                        x.Start.DateTime.HasValue
                        && x.Start.DateTime.Value.Day >= DateTime.Now.Day
                        && new[] { "Früh", "Spät" }.Any(y => x.Summary.Contains(y))
                )
                .ToList();
        }
    }
